...
class Room extends EventEmiter
{
    constructor(config = {})
    {
        ...
        this.name = config.name || 'room#' + this.UUID;
        this.size = config.size || 5;
        this.motd = config.motd || `room ${this.name}`;
        
        this.globalIO = config.io;
        this.io = this.globalIO.of(`/${this.UUID}`);

        console.log(`-- Création de la room ${this.UUID}:${this.name}`)
    ...
    initIO()
    {
        ...
        {
            console.log(`[${this.UUID}:${this.name}] [~] Connection du socket [${socket.id}] a la room`);
            ...
            socket.on('disconnect', () => 
            {
                ...
                {
                    console.log(`[${this.UUID}:${this.name}] [-] Le joueur [${this.players[socket.id].name}] a quitter la room`);
                    this.emit('playerDisconnect', this.players[socket.id].name, this);

                    delete this.players[socket.id];
                }

                console.log(`[${this.UUID}:${this.name}] [~] Déconnexion du socket [${socket.id}] de la room`);
    ...
    join(clientData = {})
    {
        if(clientData.socket)
        {
            ...
            console.log(`[${this.UUID}:${this.name}] [+] Le joueur [${this.players[clientData.socket.id].name}] a rejoin la room`);
        ...
        {
            console.log(`${clientData.name} ne peut pas rejoindre la room ${this.UUID}:${this.name} : invalid socket`);
        }
    }
